...
pub async fn create_applicant(
    ...
    for key in keys {
        let new_candles = get_candles_with_cache("XRPUSDT".to_string(), interval, key,  look_back, None).await;
    ...
    let applicant_id = save_parse_network_applicant(
        &parse, from, to, days, high_score, look_back, gain, stake, lag, interval,
    )
    .await;
